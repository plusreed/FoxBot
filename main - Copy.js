...
fox.on("message", function(message) {
  if (message.content == config.prefix + "doespixxosuck") {
    fox.reply(message, "yeah he's such a fucking faggot -_-");
  }
});
...
fox.on("message", function(message) {
  if (message.content == config.prefix + "het") {
    fox.reply(message, "Нет");
  }
});

fox.on("mention", function(message) {
  if (message.content == "brb gotta shit") {
    fox.reply(message, "ok");
  }
});
